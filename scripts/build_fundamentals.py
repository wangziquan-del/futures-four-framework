...
SURVEY_NOTES: dict[str, str] = {
    ...
    "PS": "小作文(8/16):北京协会会(盐城8家+晶诺,无政府)定不低于完全成本销售+联合减产,但130万吨配额争议大;TH追买1-2000吨(传期货配合),其余下游借关税前涨价清库存不囤货,11月后需求看淡。",
...
CONTRADICTIONS = {
    ...
    'PS': '政策协同底(协会会:不低于完全成本销售+联合减产)vs过剩现实+需求透支(关税前抢出口清库存);配额130万吨争议大、落地存疑,下游仅TH追买(传期货配合)其余不囤货。基调:政策博弈震荡,协同证真前按反弹对待,11月后需求看空。',
    ...
    'SH': '烧碱行业亏损(山东32%液碱折百1920 vs 财务成本1800-2100),边际出清线=金茂/民祥/华祥(信发自备电0.4元/度最低);降负常态化(华祥5成/鲁西7-8成/恒通7成,离子膜底线6-7成),但下半年确定性投产约100万吨(巨化45万吨8月底9月初)累库确定;氧化铝耗碱受外矿替代压制(几内亚0.05-0.08 vs 国产0.15-0.20),魏桥库存27万吨(压力线30万)安全边际充足、"量价挂钩"采购分化碱厂保价;氯碱跷跷板下液氯挺价托底(中枢150-200)。基调:短期磨底(再跌20-30到头)、远月01偏空。',
