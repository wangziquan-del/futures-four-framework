...
FOCUS: dict[str, dict[str, Any]] = {
    ...
    "AL": {"route": "al", "contradiction": "华南升水全国最强但系到货减少而非需求好;调研(8/14)确认8月消费急转弱(铝棒接单7月9万吨→8月上半月不足2万吨),链条:消费走弱→加工费下滑→铝棒减产→铝水比例调整→入库,时滞1-2个月,9月前后社库拐点风险上升;保税俄铝量大、后续进口有增量预期。海外侧印尼调研(8月):2027印尼电解铝增量下修148→135万吨,约束从ODI切换为电力(机组到货2028才是压力峰值),镍铁让电若谈成则上修,但海外2027总放量+369万吨方向不变;氧化铝先于电解铝放量(印尼2027+217万吨)压制成本端。供给天平中期倾斜,短期等社库拐点确认。", "marginal_focus": "铝棒加工费是否止跌|社库拐点(9月前后)|俄铝进口增量|印尼机组到货/镍铁让电|RKAB配额|伦铝3250-3300", "metrics": [
    ...
    "NI": {"route": "ni", "contradiction": "印尼低成本镍持续放量(供给过剩主线,成本曲线被拉低,LME失真定价在印尼);沪镍贴近印尼现金成本——过剩逻辑到成本附近自我修正,反弹即空但贴近成本收敛、不追空。边际(印尼调研8月):哈利达12万吨MHP+30万吨镍铁生产正常、硫磺库存3-4个月(中东运输受阻有加拿大补充),成本冲击未传导;镍铁让电给电解铝协商仍无结果——若落地=NPI隐性减产+铝供应上修,双向摇摆因子。", "marginal_focus": "印尼政策/新产能|镍铁让电协商|硫磺/硫酸成本|现金成本线|LME/社库|不锈钢排产", "metrics": [
        ("a10018953", "纯镍社会库存", "吨", "SMM", "国内累库是否减速"),
        ("a10193590", "印尼 NPI 产量", "万镍吨", "SMM", "边际供应是否收缩"),
        ("s20019092", "金川镍升贴水", "元/吨", "SMM", "低价下现货是否转紧")]},
    "SN": {"route": "sn", "contradiction": "矿紧未解(云南TC 17,500低位,缅甸1-5月月均进口仅约6,600吨、远低于万吨常态)+全球显性库存去化(社库7,531/LME 5,485多年低位)是高价根基;但华南调研(8/14)确认下游拒价——加工环节成本仅传导50-60%、锡价40万以下才有较强补库意愿,焊料开工率7月72.8%(6月78.8%)淡季回落。需求结构分化:消费电子量平且单耗趋势性下滑(芯片集成化),IGBT/光伏订单翻倍、军工确定性增长。价格被夹在矿紧支撑与40万补库线之间,高位区间震荡。周报(8/15)现货边际转松:江西再生锡厂停炉约一周(300-400吨)但隐形库存回升3,600吨、云锡升水回落千元、月差收敛至平水80元——区间上沿压力加大。", "marginal_focus": "缅甸月度进口是否放量(回万吨=矿紧证伪)|云南TC拐点|社库/LME去库斜率|40万下游补库线|焊料开工率8月值|佤邦复产政策|隐形库存/云锡升水(转松确认)|SOX与锡价背离", "metrics": [
